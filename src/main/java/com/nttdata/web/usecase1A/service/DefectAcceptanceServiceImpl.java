...
public class DefectAcceptanceServiceImpl implements DefectAcceptanceService {
	...
	public int[][][] processTask(String useCaseType, String predictionId, String metricsId, String userId,
		...
		List<ReleaseDetails> lastTenPredictedValues = defectAcceptanceDAO.getLastTenPredictions(algorithmId);
	...
	private int[][][] createDataSetsTelephonica(
			...
			int[] ucl, int[] lcl, int algorithmId, List<ReleaseDetails> lastTenPredictedValues) {
		...
		lclValues[defectAcceptanceModelListTelephonica.size()] = new int[] {
				defectAcceptanceModelListTelephonica.get(defectAcceptanceModelListTelephonica.size() - 1).getRelease()
						+ 1,
				lcl[0] };
		predictedArrayLinearRegression[0] = defectAcceptance[defectAcceptance.length - 1];
		predictedArrayLinearRegression[1] = new int[] {
				defectAcceptanceModelListTelephonica.get(defectAcceptanceModelListTelephonica.size() - 1).getRelease() + 1,
				oldList.get(0) };
		...
		if (algorithmId == 1) {
			...
			predictedArraySVRLinearKernel[1] = new int[] { defectAcceptanceModelListTelephonica
					.get(defectAcceptanceModelListTelephonica.size() - 1).getRelease() + 1, oldList.get(1) };

			int[][] predictedArraySVRRBFKernel = new int[2][];
			predictedArraySVRRBFKernel[0] = defectAcceptance[defectAcceptance.length - 2];
			predictedArraySVRRBFKernel[1] = new int[] { defectAcceptanceModelListTelephonica
					.get(defectAcceptanceModelListTelephonica.size() - 1).getRelease() + 1, oldList.get(2) };
