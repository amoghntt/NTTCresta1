package com.nttdata.web.usecase1A.controller;
...
import com.nttdata.web.usecase1A.service.DefectAcceptanceService;
...
@Controller
public class DefectAcceptanceController {
	
	@Autowired
	private DefectAcceptanceService defectAcceptanceService;

	@Autowired
	private CommonService commonService;

	@RequestMapping(value = "/predictDefectAcceptance", method = RequestMethod.POST)
	public ModelAndView predictUseCase1A(@ModelAttribute("configBean") ConfigBean configBean,
			HttpServletRequest request) {

		String predictionId = "4";
	...
	public ModelAndView getPredictResult(List<MetricsBean> metricsList, HttpServletRequest request,String predictionId, int algorithmId
) {
		...
		graphData = defectAcceptanceService.processTask("predictUseCase1A", predictionCode, "NA", userId,
		...
		modelView.addObject("yLabel",CrestaQueryConstants.DEFECT_ACCEPTANCE);
