...
public class DefectDensityServiceImpl implements DefectDensityService {
	...
	public int[][][] processTask(String useCaseType, String predictionId, String metricsId, String userId,
		...
		List<DefectDensityModelTelephonica> resultList = defectDensityDAO
				.getDefectDensityAndReleaseDataTelephonica(userId, predictionId);
	...
	private int[][][] createDataSetsTelephonica(List<DefectDensityModelTelephonica> defectDensityModelTelephonicaList,
		...
		uclValues[defectDensityModelTelephonicaList.size()] = new int[] {
				defectDensityModelTelephonicaList.get(defectDensityModelTelephonicaList.size() - 1).getRelease() + 1,
				ucl[0] };
		...
		if (algorithmId == 1) {
			...
			predictedArraySVRLinearKernel[1] = new int[] {
					defectDensityModelTelephonicaList.get(defectDensityModelTelephonicaList.size() - 1).getRelease()
							+ 1,
					oldList.get(1) };
...
/*	private int[][][] createDataSetsTelephonica(List<DefectDensityModelTelephonica> defectDensityModelTelephonicaList,
		...
		if (algorithmId != 1) {
			for (ReleaseDetails lastPrediction : lastTenPredictedValues) {
				lastTenPredictedArray[j] = new int[] { lastPrediction.getReleaseid(),
						lastPrediction.getPredictedValueInt() };
				j++;
			}
		}
		for (DefectDensityModelTelephonica defect : defectDensityModelTelephonicaList) {
