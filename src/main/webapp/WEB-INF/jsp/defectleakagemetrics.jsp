...
<body onload="check()">
	...
	<div id="defects" class="tabcontent">
		...
		<c:if test="${fn:length(configBean.predictBean.metricsList) gt 0}">
			...
			<form:form method="POST" action="predictDefectLeakage">
				...
				<table align="center" style="font-family: Verdana;font-weight: bold;">
					...
					<c:forEach items="${configBean.predictBean.metricsList}"
						...
						<tr>
							...
							<td><input class="button" type="button" id="calculate"
								...
								type="hidden" id="uclId${counter.count-1}1"
								name="predictBean.metricsList[${counter.count  - 1}].ucl[1]"
								value="<c:out value="${metrics.ucl[1]}"/>"> <input
								class="textb" type="hidden" id="lclId${counter.count-1}1"
								name="predictBean.metricsList[${counter.count  - 1}].lcl[1]"
								value="<c:out value="${metrics.lcl[1]}"/>"></td>
							
							<td><input class="textb" type="text"
								id="lclId${counter.count-1}0"
								name="predictBean.metricsList[${counter.count  - 1}].lcl[0]"
								value="<c:out value="${metrics.lcl[0]}"/>"> &nbsp;</td>
							<td><input class="textb" type="text"
								id="uclId${counter.count-1}0"
								name="predictBean.metricsList[${counter.count  - 1}].ucl[0]"
								value="<c:out value="${metrics.ucl[0]}"/>"></td>
							<td>
								<div id="weightageDiv">
									<input class="textb" type="text"
										id="weightageId${counter.count-1}"
										name="predictBean.metricsList[${counter.count  - 1}].weightage"
										value="<c:out value="${metrics.weightage}"/>">
								</div>
